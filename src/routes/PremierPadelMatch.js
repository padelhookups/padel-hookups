...
const PremierPadelMatch = () => {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(
      ...
      (snapshot) => {
        if (snapshot.exists()) {
          const updatedMatch = { id: snapshot.id, ...snapshot.data() };
          setMatch(updatedMatch);
          
          // Update summary fields if they were set by this or the other team
          if (updatedMatch.ChoosenDate) setSummaryDate(updatedMatch.ChoosenDate);
          if (updatedMatch.ChoosenTime) setSummaryTime(updatedMatch.ChoosenTime);
          if (updatedMatch.location) setSummaryLocation(updatedMatch.location);
        }
      },
      (error) => {
  ...
  useEffect(() => {
    if (match && event && user) {
      ...
      const teamAIds = [teamAPair?.Player1Id, teamAPair?.Player2Id].filter(
        Boolean,
      );

      const teamBIds = [teamBPair?.Player1Id, teamBPair?.Player2Id].filter(
        Boolean,
      );
  ...
  return (
    <>
      ...
      <Box sx={{ backgroundColor: BG }}>
        ...
        >
          <Tab label="Summary" />
          {user?.isAdmin || currentTeam ? <Tab label="Schedule" /> : null}
          <Tab label="Results" />
        </Tabs>
            ...
            {activeTab === 1 && (
              <>
                <Schedule
                  ...
                  onConfirmed={async (date, time, overlaps, chosenOverlaps) => {
                    ...
                    const finalOverlaps = overlaps
                      .map((o) => {
                        if (o.date === chosenOverlaps.date) {
                          // Remove the chosen slot from this date's slots
                          const filteredSlots = o.slots.filter(
                            (slot) => slot !== chosenOverlaps.slot,
                          );
                          return {
                            date: o.date,
                            slots: filteredSlots,
