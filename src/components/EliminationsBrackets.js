 ...
 */
			const groupStageData = await manager.get.getStageSpecificData(
				groupStage.id
			);
			...
			const rankingFormula = (participant) => {
				...
				const groupIndex = Object.values(participantGroups).findIndex(
					(group) => group.includes(participant.id)
				);
				...
				for (const other of tied) {
					const directMatch = eliminationStageData.matches.find(
						(m) =>
							...
							[m.opponent1?.id, m.opponent2?.id].includes(
								participant.id
							) &&
							[m.opponent1?.id, m.opponent2?.id].includes(
								other.id
			...
			const selectedStageData = showGroups
				? groupStageData
				: eliminationStageData;

			// 1) Ordenar os rounds pelo campo "number"
			if (selectedStageData.rounds) {
				selectedStageData.rounds = [...selectedStageData.rounds].sort(
			...
			const orderedMatches = [...(selectedStageData.matches || [])].sort(
				(a, b) => {
			...
			console.log(
				"matchGames",
				showGroups ? [] : selectedStageData.matchGames
			);
			...
			window.bracketsViewer.render(
				{
					...
					matchGames: showGroups
						? []
						: selectedStageData.matchGames || [],
				...
				<Paper elevation={1} sx={{ p: 2.5 }}>
					<Stack spacing={1.25}>
						<Typography
							...
							textAlign='center'>
							No groups available
							<br /> Create the pairs and then generate the
							tournament groups in details page
						</Typography>
					</Stack>
				</Paper>
			</Container>
			...
			<div
				...
				<Stack direction='row' justifyContent='center' sx={{ mb: 2 }}>
					<ToggleButtonGroup
						...
						onChange={(_, value) => {
							if (!value) return;
							if (value === "elimination" && !hasEliminationStage)
								return;
