...
const EventRankings = ({ eventId, tournamentId }) => {
    ...
    useEffect(() => {
        const fetchTournamentData = async () => {
            ...
            const finalRankings = await getRankings(tempMatches, stageData.participant);
    ...
    function getRankings(matches, participants = []) {
        ...
        for (const m of matches) {
            ...
            if (m.status === 4) { // apenas jogos completos
        ...
        function computeDirectMini(ids) {
            const mini = {};
            for (const id of ids) mini[id] = { wins: 0, pointsFor: 0, pointsAgainst: 0 };

            for (const m of matches) {
                if (m.status !== 4 || !m.opponent1 || !m.opponent2) continue;
                const a = m.opponent1.id, b = m.opponent2.id;
                if (!ids.includes(a) || !ids.includes(b)) continue; // só entre empatados
                const sA = m.scoreTeam1, sB = m.scoreTeam2;
        ...
        function sortTeams(teams) {
            ...
            const winsKeys = Object.keys(groups).map(Number).sort((a, b) => b - a);
            ...
            for (const w of winsKeys) {
                ...
                const pointsKeys = Object.keys(pointsMap).map(Number).sort((a, b) => b - a);
    ...
    return (
        <Paper elevation={1} sx={{ p: 2.5 }}>
            <Stack spacing={1.25}>
                {pairs?.map((pair, idx) => {
                    const pairName1 = participants.find(p => p.id === pair.id)?.name.split(' & ')[0] || 'Unknown';
                    const pairName2 = participants.find(p => p.id === pair.id)?.name.split(' & ')[1] || 'Unknown';

                    return (
                        <Stack key={pair.id} direction="row" alignItems="center" justifyContent="space-between">
                            <Stack direction="row" spacing={1.25} alignItems="center">
                                <Chip size="small" label={`#${idx + 1}`} />
                                <Stack direction="row" spacing={1} alignItems="center">
                                    <Avatar sx={{ width: 28, height: 28, bgcolor: 'primary.main', fontSize: 12 }}>{initials(pairName1)}</Avatar>
                                    <Avatar sx={{ width: 28, height: 28, bgcolor: 'primary.main', fontSize: 12 }}>{initials(pairName2)}</Avatar>
                            ...
                            <Stack direction="row" spacing={1} alignItems="center">
                                <Chip
                                    color="primary"
                                    label={<Typography variant="body2" sx={{ color: "white", fontWeight: 'bold' }}>{`${pair.wins} wins`}</Typography>} />
                                <Chip variant="outlined" label={`${pair.points} pts`} size="small" />
                                {
                                    pair.miniWins !== undefined && pair.miniDiff !== undefined ? (
                                        <>
                                            <Chip variant="outlined" label={`${pair.miniWins} Direct Wins`} size="small" title="Represents how many games a team has won against the other tied teams" />
                                            <Chip variant="outlined" label={`${pair.miniDiff} Direct Diff`} size="small" title="Represents the difference in points (scored – conceded) only between teams tied on points" />
                                        </>
                                    ) : null}
                            </Stack>
                        </Stack>);
